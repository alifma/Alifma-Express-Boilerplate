...
module.exports = {
  ...
  // Update User
  cUpdateUser: async (req, res) => {
    try {
      const id = req.params.id
      const data = req.body
      // Semua kolom yang bisa diperbaharui dari Database
      if (req.file || data.username || data.first_name || data.last_name || data.email || data.password) {
        let dataUpdate = {}
        ...
        if (req.file) {
          ...
          mDetailUser(id)
            .then((res) => {
              if (res[0].image !== 'userdefault.png') {
        ...
        if (data.email) {
          await mCheckEmail(data.email)
            .then(async (resCheck) => {
              await mDetail(id)
                .then(async (resDetail) => {
                  if (resCheck.length >= 1 && (resCheck[0].email != resDetail[0].email)) {
                    // Kalau email sudah dipakai sama yang lain
                    if (req.file) {
                      fs.unlinkSync(`./public/img/${req.file.filename}`)
                    }
                    error(res, 400, 'Email already Used!', 'Please use your old email or another email!', {})
                  } else {
                    // Kalau emailnya pakai email lama atau pakai email baru
                    await mUpdateUser(dataUpdate, id)
                      .then((response) => {
                        if (response.affectedRows != 0) {
                          success(res, 200, 'Update data Success', {}, {})
                        } else {
                          if (req.file) {
                            fs.unlinkSync(`./public/img/${req.file.filename}`)
                          }
    ...
    try {
      ...
      await jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        if (err) {
          ...
          mDetail(decoded.id)
            .then((responseDetail) => {
              success(res, 200, 'Token Verified!', {}, {
                id: responseDetail[0].id
              })
            })
            .catch((err) => {
              error(res, 500, 'Internal Server Error', err.message, {})
